...
def render_test_execution():
    ...
    with get_db() as db:
        service_requests = db.execute(select(ServiceRequest).where(
...
def render_test_results_checksheet():
    ...
    try:
        with get_db() as db:
            active_executions = db.execute(select(TestExecution).where(
            ...
            completed_executions = db.execute(select(TestExecution).where(
                ...
                with st.expander(f"🔬 {execution.execution_number} - {execution.sample_id or 'No Sample'} ({execution.status.value})", expanded=True):
                    ...
                    with st.form(f"update_{execution.id}"):
                        ...
                        if start_test:
                            try:
                                with get_db() as db:
                                    exec_record = db.execute(select(TestExecution).where(TestExecution.id == execution.id)).scalars().first()
                    ...
                    try:
                        with get_db() as db:
                            data_points = db.execute(select(TestData).where(
