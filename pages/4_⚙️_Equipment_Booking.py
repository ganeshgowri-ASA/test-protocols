            ...
            if submitted:
                ...
                try:
                    ...
                    with get_db() as db:
                        ...
                        equipment = db.execute(select(Equipment).filter(Equipment.id == equipment_id)).scalar_one_or_none()
...
def render_bookings_list():
    ...
    try:
        ...
        with get_db() as db:
            bookings = db.execute(select(EquipmentBooking).options(
                joinedload(EquipmentBooking.equipment)
            ).filter(
                EquipmentBooking.is_active == True
            ).order_by(EquipmentBooking.start_time.desc()).limit(20)).scalars().unique().all()
            ...
            with st.expander(
                ...
                with col1:
                    if st.button("✅ Complete Booking", key=f"complete_{booking_info['id']}"):
                        with get_db() as db:
                            booking = db.execute(select(EquipmentBooking).filter(
                                EquipmentBooking.id == booking_info['id']
                            )).scalar_one_or_none()
                            if booking:
                                booking.is_active = False
                                booking.actual_end_time = datetime.now()
                                equipment = db.execute(select(Equipment).filter(
