            ...
            if submitted:
                ...
                try:
                    ...
                    with get_db() as db:
                        ...
                        equipment = db.execute(select(Equipment).where(Equipment.id == equipment_id)).scalars().first()
...
def render_bookings_list():
    ...
    try:
        ...
        with get_db() as db:
            ...
            ).order_by(EquipmentBooking.start_time.desc()).limit(20)).scalars().all()
            ...
            with st.expander(
                ...
                with col1:
                    if st.button("✅ Complete Booking", key=f"complete_{booking_info['id']}"):
                        with get_db() as db:
                            booking = db.execute(select(EquipmentBooking).where(
                                EquipmentBooking.id == booking_info['id']
                            )).scalars().first()
                            if booking:
                                booking.is_active = False
                                booking.actual_end_time = datetime.now()
                                equipment = db.execute(select(Equipment).where(
