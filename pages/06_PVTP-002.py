"""
PVTP-002: Thermal Cycling Test Protocol
Testing module resistance to thermal stress through temperature cycling
"""
...
# Page Configuration
st.set_page_config(
    page_title="PVTP-002: Thermal Cycling",
    page_icon="🌡️",
    layout="wide"
)
...
if 'execution_id_tc' not in st.session_state:
    st.session_state.execution_id_tc = None
if 'auto_save' not in st.session_state:
    st.session_state.auto_save = True
if 'current_status_tc' not in st.session_state:
    st.session_state.current_status_tc = 'not_started'

# ============================================
# PAGE HEADER
# ============================================

st.title("🌡️ PVTP-002: Thermal Cycling Test")
st.markdown("### Temperature Cycle Stress Testing per IEC 61215-2")
...
with col1:
    st.metric("Protocol", "PVTP-002")
with col2:
    st.metric("Status", f"{status_colors.get(st.session_state.current_status_tc, '⚪')} {st.session_state.current_status_tc.replace('_', ' ').title()}")
with col3:
    if st.session_state.execution_id_tc:
        st.metric("Execution ID", st.session_state.execution_id_tc)
    else:
...
with tab1:
    ...
    with st.form("general_data_form_tc"):
        ...
        with col1:
            ...
            sample_id = st.text_input(
                "Sample ID *",
                placeholder="e.g., PV-2024-002",
                help="Unique identifier for the test sample"
            )

            equipment_id = st.text_input(
                "Equipment ID *",
                placeholder="e.g., TC-CHAMBER-01",
                help="Thermal cycling chamber identifier"
            )
        ...
        with col2:
            ...
            estimated_duration = st.number_input(
                ...
                value=15.0,
                step=1.0,
                help="Typical: 10-20 days for 200-400 cycles"
            )
    ...
    if save_general:
        if not sample_id or not equipment_id or not operator:
            ...
            try:
                ...
                if not st.session_state.execution_id_tc:
                    execution_data = {
                        'protocol_id': 'PVTP-002',
                    ...
                    st.session_state.execution_id_tc = execution_id
                    st.session_state.current_status_tc = 'not_started'
...
with tab2:
    st.subheader("Thermal Cycling Test Parameters")

    if not st.session_state.execution_id_tc:
        st.warning("⚠️ Please complete General Data first to enable protocol inputs.")
    else:
        with st.form("protocol_inputs_form_tc"):
            ...
            with col2:
                st.markdown("#### Thermal Cycle Parameters")
                ...
                number_of_cycles = st.number_input(
                    ...
                    value=200,
                    step=50,
                    help="IEC 61215: 200 cycles standard, 400+ for extended testing"
                )

                low_temperature = st.number_input(
                    "Low Temperature (°C) *",
                    min_value=-80.0,
                    max_value=0.0,
                    value=-40.0,
                    step=5.0,
                    help="Typical: -40°C per IEC 61215"
                )

                high_temperature = st.number_input(
                    "High Temperature (°C) *",
                    min_value=50.0,
                    max_value=150.0,
                    value=85.0,
                    step=5.0,
                    help="Typical: 85°C per IEC 61215"
                )

                dwell_time_low = st.number_input(
                    "Dwell Time at Low Temp (min)",
                    min_value=5,
                    max_value=120,
                    value=15,
                    step=5,
                    help="Time at low temperature"
                )

                dwell_time_high = st.number_input(
                    "Dwell Time at High Temp (min)",
                    min_value=5,
                    max_value=120,
                    value=15,
                    step=5,
                    help="Time at high temperature"
                )
            ...
            st.markdown("#### Intermediate Testing Points")
            ...
            with col1:
                test_at_cycles = st.multiselect(
                    ...
                    options=[0, 10, 50, 100, 150, 200, 250, 300, 400],
                    default=[0, 50, 100, 200],
                    help="Cycles at which to perform intermediate measurements"
                )

            with col2:
                visual_inspection_cycles = st.multiselect(
                    "Visual Inspection at Cycles:",
                    options=[0, 50, 100, 150, 200, 250, 300, 400],
                    default=[0, 100, 200],
                    help="Cycles at which to perform visual inspection"
                )

            with col3:
                insulation_test_cycles = st.multiselect(
                    "Insulation Test at Cycles:",
                    options=[0, 50, 100, 150, 200, 250, 300, 400],
                    default=[0, 100, 200],
                    help="Cycles at which to test insulation resistance"
                )
            ...
            with col3:
                min_insulation = st.number_input(
                    ...
                    value=40.0,
                    step=5.0,
                    help="Minimum acceptable insulation resistance"
                )
            ...
            with col3:
                frame_type = st.selectbox(
                    "Frame Type",
                    options=["Aluminum", "Frameless", "Steel", "Composite"]
                )
                ...
                try:
                    ...
                    if start_test:
                        st.session_state.db.update_protocol_status(
                            st.session_state.execution_id_tc,
                            'in_progress',
                            1
                        )
                        st.session_state.current_status_tc = 'in_progress'
                        st.success("✅ Thermal cycling test started!")
                    else:
...
with tab3:
    ...
    if not st.session_state.execution_id_tc or st.session_state.current_status_tc == 'not_started':
        ...
        with col1:
            if st.button("▶️ Resume", use_container_width=True, key="resume_tc"):
                st.session_state.db.update_protocol_status(
                    st.session_state.execution_id_tc,
                    'in_progress',
                    1
        ...
        with col2:
            if st.button("⏸️ Pause", use_container_width=True, key="pause_tc"):
                st.session_state.db.update_protocol_status(
                    st.session_state.execution_id_tc,
                    'paused',
                    1
                )
                st.session_state.current_status_tc = 'paused'
                st.rerun()

        with col3:
            if st.button("✅ Complete", use_container_width=True, key="complete_tc"):
                st.session_state.db.update_protocol_status(
                    st.session_state.execution_id_tc,
                    'completed',
                    1
                )
                st.session_state.current_status_tc = 'completed'
                st.rerun()

        with col4:
            if st.button("🔄 Refresh Data", use_container_width=True, key="refresh_tc"):
                st.rerun()

        st.markdown("---")

        # Current cycle status (simulated)
        st.markdown("### 🔄 Current Cycle Status")

        col1, col2, col3, col4 = st.columns(4)

        with col1:
            st.metric("Current Cycle", "87/200")
        with col2:
            st.metric("Chamber Temp", "65.3 °C")
        with col3:
            st.metric("Time in Cycle", "8 min")
        with col4:
            st.metric("Est. Completion", "7 days")

        # Progress bar
        cycle_progress = 87 / 200 * 100
        ...
        with st.expander("➕ Add Measurement Reading", expanded=False):
            with st.form("add_measurement_tc"):
                col1, col2, col3 = st.columns(3)

                with col1:
                    meas_cycle = st.number_input("Cycle Number", min_value=0, max_value=1000, value=0, step=1)
                ...
                with col3:
                    visual_defects = st.multiselect(
                        "Visual Defects",
                        options=["None", "Delamination", "Cracks", "Discoloration", "Bubbles", "Corrosion"]
                    )
                ...
                if st.form_submit_button("💾 Save Measurement"):
                    try:
                        measurement_data = {
                            'execution_id': st.session_state.execution_id_tc,
                            'measurement_type': 'thermal_cycle_reading',
        ...
        try:
            measurements = st.session_state.db.get_measurements(st.session_state.execution_id_tc)
            ...
            if measurements:
                ...
                if len(df) > 0:
                    ...
                    fig_deg.update_layout(
                        title='Power Degradation vs Thermal Cycles',
                        xaxis_title='Cycle Number',
                    ...
                    with col1:
                        fig_voc = px.line(df, x='Cycle', y='Voc (V)',
                                         title='Voc vs Thermal Cycles',
                                         markers=True)
                        fig_voc.update_traces(line_color='green')
                        st.plotly_chart(fig_voc, use_container_width=True)

                        fig_ff = px.line(df, x='Cycle', y='FF (%)',
                                        title='Fill Factor vs Thermal Cycles',
                                        markers=True)
                        fig_ff.update_traces(line_color='darkorange')
                        st.plotly_chart(fig_ff, use_container_width=True)

                    with col2:
                        fig_isc = px.line(df, x='Cycle', y='Isc (A)',
                                         title='Isc vs Thermal Cycles',
                                         markers=True)
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name=f"PVTP002_ThermalCycle_Data_{st.session_state.execution_id_tc}.csv",
                        mime="text/csv"
                    )
            else:
    ...
    if not st.session_state.execution_id_tc:
        ...
        try:
            ...
            if measurements and len(measurements) >= 2:
                ...
                with col2:
                    st.markdown("#### Visual Defects")
                    # Count defects
                    defect_count = 0
                    for record in data_records:
                        defects = record.get('visual_defects', [])
                ...
                if degradation_pct < -10:
                    st.error("❌ Severe degradation. Module unsuitable for deployment.")
                elif degradation_pct < -5:
                    st.warning("⚠️ Exceeds IEC limit. Further investigation recommended.")
                elif degradation_pct < -2:
                    st.info("ℹ️ Minor degradation within limits.")
                else:
                    st.success("✅ Excellent thermal cycling performance.")
...
with tab5:
    ...
    if not st.session_state.execution_id_tc:
        ...
        qc_checks = [
            ...
            {"id": "qc_02", "name": "Sample secured properly", "stage": "before_test"},
            {"id": "qc_03", "name": "Initial measurements completed", "stage": "before_test"},
            {"id": "qc_04", "name": "Temperature profile validated", "stage": "during_test"},
            {"id": "qc_05", "name": "No chamber alarms/errors", "stage": "during_test"},
            {"id": "qc_06", "name": "Intermediate tests performed", "stage": "during_test"},
            {"id": "qc_07", "name": "Final measurements completed", "stage": "after_test"},
            {"id": "qc_08", "name": "Visual inspection passed", "stage": "after_test"},
            {"id": "qc_09", "name": "Data integrity verified", "stage": "after_test"},
        ]
        ...
        for check in qc_checks:
            ...
            with col3:
                qc_status = st.selectbox(
                    ...
                    key=f"qc_tc_{check['id']}",
                    label_visibility="collapsed"
                )

        if st.button("💾 Save QC Checkpoints", key="save_qc_tc"):
        ...
        with col1:
            if st.button("📄 Generate Report", use_container_width=True, key="gen_report_tc"):
                with st.spinner("Generating report..."):
                    st.success(f"✅ {report_type} generated successfully!")
                    st.info(f"Report ID: RPT-{datetime.now().strftime('%Y%m%d%H%M%S')}")

        with col2:
            if st.button("📧 Email Report", use_container_width=True, key="email_report_tc"):
                st.success("✅ Report sent to stakeholders!")

        with col3:
            if st.button("💾 Archive", use_container_width=True, key="archive_tc"):
...
with st.sidebar:
    ...
    st.info("""
    **PVTP-002: Thermal Cycling**

    **Purpose:**
    Evaluate module resistance to thermal stress through repeated temperature cycling
    ...
    **Test Duration:**
    10-20 days typical for 200 cycles

    **Key Parameters:**
    - Temperature range: -40°C to +85°C
    - 200 cycles (standard)
    - Intermediate I-V measurements
    - Visual inspection
    ...
    if st.button("🏠 Home", use_container_width=True, key="nav_home_tc"):
        st.switch_page("streamlit_app.py")

    if st.button("📋 Service Requests", use_container_width=True, key="nav_sr_tc"):
        st.switch_page("pages/01_Service_Request.py")

    if st.button("🎯 Protocol Selector", use_container_width=True, key="nav_ps_tc"):
    ...
    if st.session_state.execution_id_tc:
        ...
        if st.button("🗑️ Reset Session", use_container_width=True, key="reset_tc"):
            st.session_state.execution_id_tc = None
            st.session_state.current_status_tc = 'not_started'
            st.rerun()
    ...
    # Auto-save toggle
    st.markdown("---")
    auto_save = st.checkbox("🔄 Auto-save", value=st.session_state.auto_save, key="auto_save_tc")
